...
public class MermodRenderTypes {
    ...
    static {
        ...
        GLINT_CULL_PIPELINE = RenderPipeline.builder(RenderPipelines.MATRICES_COLOR_SNIPPET, RenderPipelines.FOG_NO_COLOR_SNIPPET).withLocation(ResourceLocation.fromNamespaceAndPath(Mermod.MOD_ID, "pipeline/glint")).withVertexShader("core/glint").withFragmentShader("core/glint").withSampler("Sampler0").withUniform("TextureMat", UniformType.MATRIX4X4).withUniform("GlintAlpha", UniformType.FLOAT).withDepthWrite(false).withCull(true).withDepthTestFunction(DepthTestFunction.EQUAL_DEPTH_TEST).withBlend(BlendFunction.GLINT).withVertexFormat(DefaultVertexFormat.POSITION_TEX, VertexFormat.Mode.QUADS).build();
